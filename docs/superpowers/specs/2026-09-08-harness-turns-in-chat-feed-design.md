...
| (new)                                                                                  | `apps/server/src/chat/feed-cursor.ts`: the ordering primitives (`SOURCE_RANK`, `cursorClause`, `Keyed`, the cursor codec) that `feed.ts` and `chat/turns.ts` both import, so neither imports the other; `feed.ts` re-exports what its callers used |
