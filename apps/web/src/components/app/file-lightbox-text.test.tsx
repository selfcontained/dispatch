...
import { MarkdownViewer } from "@/components/app/file-lightbox-text";
...
describe("MarkdownViewer", () => {
  ...
  it("keeps the same scroll container mounted when the same file refreshes", async () => {
    ...
    const { container, rerender } = render(
      <MarkdownViewer src="/files/report.md?t=1" fileName="report.md" />,
      { wrapper }
    );
    ...
    rerender(
      <MarkdownViewer src="/files/report.md?t=2" fileName="report.md" />
    );
  ...
  it("shows the loading state when navigating to a different file", async () => {
    ...
    rerender(<MarkdownViewer src="/files/other.md?t=1" fileName="other.md" />);
