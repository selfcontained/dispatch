...
/** Source tag stored alongside an uploaded file. */
...
export type UploadedFile = {
   ...
   * depend on it from other file consumers.
   */
  path: string;
...
/**
 * Upload a single file to an agent's file store. Writes the file on the
 * server (under the agent's files dir), records it in the files table, and
...
export async function uploadAgentFile(
  agentId: string,
  file: File,
  opts: { source?: FileUploadSource; inject?: boolean } = {}
): Promise<UploadedFile> {
  const form = new FormData();
  form.append("source", opts.source ?? "user");
  if (opts.inject) form.append("inject", "true");
  form.append("file", file, file.name);
  const res = await api<{ ok: true; file: UploadedFile }>(
...
export { extensionForMime } from "../../../server/src/shared/file-types";
