...
import { normalizeDispatchBaseUrl } from "./dispatch-url";

export const CONNECTION_KEY = "dispatchConnection";
...
export type DeviceNameBuilder = (os: string, suffix: string) => string;
...
export async function getConnection(): Promise<StoredConnection | null> {
  const stored = await api.storage.local.get(CONNECTION_KEY);
  return (stored[CONNECTION_KEY] as StoredConnection | undefined) ?? null;
}

async function getDeviceName(buildName: DeviceNameBuilder): Promise<string> {
  const stored = await api.storage.local.get(DEVICE_NAME_KEY);
  ...
  await api.storage.local.set({ [DEVICE_NAME_KEY]: name });
  return name;
}
...
export async function authenticatedFetch<T>(
  ...
  try {
    ...
    if (error instanceof HttpStatusError && error.status === 401) {
      await api.storage.local.remove(CONNECTION_KEY);
    }
    throw error;
  }
}
...
export async function handleWorkerRequest(
  ...
  switch (request.type) {
    ...
    case "pairing:start": {
      ...
      const deviceName = await getDeviceName(buildName);
...
export function toErrorResponse(error: unknown): WorkerResponse {
  return {
    ok: false,
