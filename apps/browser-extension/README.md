...
The extension uses `activeTab` so it can inspect only a page on which the user
explicitly invokes it. Access to the configured Dispatch origin is requested at
pairing time. It removes form values, editable content, scripts, inline event
handlers, `srcdoc`, and likely secret URL parameters before showing or sending
the context preview.
