/**
 * The placeholder images the dev seed writes to disk, and the file rows in
 * seed/files.ts that describe them — one table so the two cannot drift.
 *
 ...
 * from the dimensions on its file row, so a 1x1 placeholder would seed a
 ...
 * seed/files.ts measures them off these bytes rather than taking a declared
