...
export function buildHarnessPersona(input: {
  ...
  const sections = [
    ...
    "For work with multiple steps, use dispatch_update_tasks to publish a short task list before starting. Send the full updated list as work progresses, marking tasks in_progress or completed. Keep it current when the user changes scope. Skip the task list for simple questions or one-step actions. This list is shown above the chat composer. Before you end a turn, send the list one last time so it matches where things stand: mark finished tasks completed, and send an empty list if the work is done or dropped. A task still in_progress when the turn ends is shown as pending, because nothing runs between turns.",
  ];
