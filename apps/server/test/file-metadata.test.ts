...
import {
  dimensionFields,
  fileMetadataFromBuffer,
  parseFileMetadata,
} from "../src/files/metadata.js";
...
describe("parseFileMetadata", () => {
  it("reads a well-formed pair", () => {
    expect(parseFileMetadata({ width: 120, height: 90 })).toEqual({
      width: 120,
      height: 90,
    });
  });
  ...
  it.each([
    ...
    expect(parseFileMetadata({ width, height: 90 })).toEqual({});
  });
  ...
  it("keeps keys it has not been taught about", () => {
    ...
    expect(
      parseFileMetadata({ width: 120, height: 90, duration: 4.5 })
  ...
  it("does not invent a dimension from a misspelled key", () => {
    ...
    const parsed = parseFileMetadata({ widht: 120, height: 90 });
...
describe("fileMetadataFromBuffer", () => {
  it("measures an image", () => {
    expect(fileMetadataFromBuffer(PNG_37x19)).toEqual({
      width: 37,
      height: 19,
    });
  });
  ...
  it.each([
    ...
    expect(fileMetadataFromBuffer(buffer)).toEqual({});
  });
});
