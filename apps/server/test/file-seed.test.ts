...
import { timestampFileName } from "../src/agents/file-seed.js";
...
describe("timestampFileName", () => {
  ...
  it("uses 1-based index suffix (the loop passes 0-based)", () => {
    expect(timestampFileName("a.png", FIXED_DATE, 0)).toContain("-1.png");
    expect(timestampFileName("a.png", FIXED_DATE, 1)).toContain("-2.png");
    expect(timestampFileName("a.png", FIXED_DATE, 9)).toContain("-10.png");
  });

  it("preserves multi-character extensions", () => {
    expect(timestampFileName("note.md", FIXED_DATE, 0)).toMatch(/\.md$/);
    expect(timestampFileName("data.tar.gz", FIXED_DATE, 0)).toMatch(/\.gz$/);
  });

  it("handles files without an extension", () => {
    const out = timestampFileName("README", FIXED_DATE, 0);
  ...
  it("preserves dots inside the basename (e.g. `notes.v2.txt`)", () => {
    ...
    // the basename stays.
    const out = timestampFileName("notes.v2.txt", FIXED_DATE, 0);
