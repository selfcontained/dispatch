...
describe("POST /api/v1/agents/:id/prompts/pin/:pinId", () => {
  it("returns 404 for an unknown agent", async () => {
    const res = await authedInject(
      "POST",
      "/api/v1/agents/agt_missing/prompts/pin/p1"
    );
  ...
  it("returns 404 when the pin ID does not exist on the agent", async () => {
    ...
    const res = await authedInject(
      "POST",
      `/api/v1/agents/${agent.id}/prompts/pin/not-a-real-pin`
    );
    expect(res.statusCode).toBe(404);
  ...
  it("refuses to run a pin that is not a shortcut", async () => {
    ...
    const res = await authedInject(
      "POST",
      `/api/v1/agents/${agent.id}/prompts/pin/p1`
    );
  ...
  it("rejects an unauthenticated request", async () => {
    ...
    const res = await ctx.app.inject({
      method: "POST",
      url: `/api/v1/agents/${agent.id}/prompts/pin/p1`,
    });
