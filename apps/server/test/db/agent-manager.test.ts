...
describe("AgentManager", () => {
  ...
  describe("getReviewerRecheckContext", () => {
    it("returns the latest resolution and its round items", async () => {
      const parentDir = await mkdtemp(
        path.join(os.tmpdir(), "dispatch-parent-")
      );
      const reviewerDir = await mkdtemp(
        path.join(os.tmpdir(), "dispatch-reviewer-")
      );
      const parent = await manager.createAgent({
        name: "parent",
        cwd: parentDir,
      ...
      await manager.submitFeedback(reviewer.id, {
        severity: "high",
        description: "First finding",
      });
      const secondFeedback = await manager.submitFeedback(reviewer.id, {
        severity: "low",
        description: "Second finding",
      });
      await manager.updateFeedbackStatus(
        secondFeedback.id,
        reviewer.id,
        "ignored",
        {
          reason: "out of scope",
          resolutionCommit: "def456",
        }
      );
      const allFeedback = await pool.query<{ id: number }>(
        `SELECT id FROM agent_feedback WHERE agent_id = $1 ORDER BY id ASC`,
        [reviewer.id]
      );
      await manager.updateFeedbackStatus(
        allFeedback.rows[0]!.id,
        reviewer.id,
        "fixed",
        { resolutionCommit: "def456" }
      );
      await manager.submitReviewResolution({
        parentAgentId: parent.id,
        personaAgentId: reviewer.id,
        summary: "Addressed the findings",
        resolutionCommit: "def456",
      });

      const context = await manager.getReviewerRecheckContext(reviewer.id);

      expect(review.id).toBeGreaterThan(0);
      expect(context?.review.status).toBe("awaiting_recheck");
      expect(context?.resolution.summary).toBe("Addressed the findings");
      expect(context?.resolution.resolutionCommit).toBe("def456");
      expect(context?.resolutions).toHaveLength(2);
      expect(context?.resolutions[0]?.originalDescription).toBe(
        "First finding"
      );
      expect(context?.resolutions[1]?.reason).toBe("out of scope");
