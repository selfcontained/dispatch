...
test.describe("Agent-authored sidebar surfaces", () => {
  ...
  test("closing the unpinned sidebar leaves the app layout in place", async ({
    page,
    request,
  }) => {
    const agent = await createAgentViaAPI(request, {
      name: `e2e-agent-surfaces-noshift-${Date.now()}`,
      cwd: process.cwd(),
    });
    await seedSurfaces(agent.id);
    await loadApp(page);
    ...
    const leftEdges = () =>
      page.evaluate(() => {
        ...
        return {
          ...
          rowScrollLeft: main.parentElement!.scrollLeft,
        };
      });
    const before = await leftEdges();
    expect(before.rowScrollLeft).toBe(0);

    // The closed drawer parks off-canvas inside the app row. If that row were
    // scrollable, keeping the active surface tab in view would scroll it and
    // drag the whole app sideways — the regression this guards.
    await page
      .getByTestId("media-sidebar")
    ...
    await page
      .getByTestId("surface-tab-button")
      .first()
      .evaluate((node) =>
        node.scrollIntoView({ block: "nearest", inline: "nearest" })
      );

    expect(await leftEdges()).toEqual(before);
  });
});
