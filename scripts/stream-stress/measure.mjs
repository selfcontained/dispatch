...
await context.addInitScript(() => {
  ...
  window.EventSource = class extends Native {
    constructor(...a) {
      ...
      this.addEventListener("message", (event) => {
        ...
        if (event.data.startsWith('{"type":"stream.entry"')) stats.entryBytes += event.data.length;
      });
    }
  };
  ...
  new PerformanceObserver((list) => {
    for (const e of list.getEntries()) window.__events.push({ name: e.name, duration: e.duration });
...
if (Number(args.cpu ?? 1) > 1) await cdp.send("Emulation.setCPUThrottlingRate", { rate: Number(args.cpu) });

async function memory(gc = true) {
  if (gc) await cdp.send("HeapProfiler.collectGarbage");
  const heap = await cdp.send("Runtime.getHeapUsage");
  const dom = await cdp.send("Memory.getDOMCounters");
  const rows = await page.evaluate(() => document.querySelectorAll("[data-testid=chat-scroll] [data-chat-entry-id]").length);
  return { heapMB: +(heap.usedSize / 1048576).toFixed(1), domNodes: dom.nodes, rows };
}
...
async function openRoot() {
  ...
  await page.waitForFunction(() => document.querySelectorAll("[data-testid=chat-scroll] [data-chat-entry-id]").length > 20, null, { timeout: 60_000 });
...
async function loadOlder(times) {
  for (let i = 1; i <= times; i += 1) {
    const before = await page.evaluate(() => document.querySelectorAll("[data-testid=chat-scroll] [data-chat-entry-id]").length);
    const button = page.locator("[data-testid=chat-scroll] button", { hasText: "Load older" });
    if ((await button.count()) === 0) break;
    const t0 = Date.now();
    await button.click();
    await page.waitForFunction((n) => document.querySelectorAll("[data-testid=chat-scroll] [data-chat-entry-id]").length > n, before, { timeout: 60_000 });
    const ms = Date.now() - t0;
    await sleep(800);
...
async function interactions(tag) {
  ...
  if (await input.count()) {
    await input.click();
    const t0 = Date.now();
    await page.keyboard.type("checking the stream while it runs", { delay: 30 });
    result.typeWallMs = Date.now() - t0;
    await sleep(300);
    const keys = await page.evaluate(() => window.__events.filter((e) => e.name === "keydown" || e.name === "keypress" || e.name === "input").map((e) => e.duration));
  ...
  await page.evaluate(() => {
    ...
    const tick = (now) => {
      ...
      last = now;
      if (window.__frames.length < 600) window.__raf = requestAnimationFrame(tick);
    };
  ...
  const fold = page.locator("[data-testid=chat-scroll] [data-testid=harness-activity-fold]").last();
  if (await fold.count()) {
    await fold.scrollIntoViewIfNeeded();
    const before = await page.locator("[data-testid=harness-step]").count();
    const t0 = Date.now();
    await fold.click();
    await page.waitForFunction((n) => document.querySelectorAll("[data-testid=harness-step]").length !== n, before, { timeout: 10_000 }).catch(() => {});
  ...
  if (finding) {
    ...
    await page
      .waitForFunction(() => document.querySelectorAll("[data-testid=chat-thread-scroll] [data-chat-entry-id]").length > 10, null, { timeout: 20_000 })
      .catch(() => {});
    result.openThreadMs = Date.now() - t0;
    result.threadRows = await page.evaluate(() => document.querySelectorAll("[data-testid=chat-thread-scroll] [data-chat-entry-id]").length);
    await sleep(500);
    await page.evaluate(() => {
      const url = new URL(location.href);
  ...
  if (child) {
    ...
    await page.waitForFunction((id) => location.pathname.includes(id) && document.querySelector("[data-testid=chat-scroll]"), child, { timeout: 20_000 });
    ...
    await page.waitForFunction(() => document.querySelectorAll("[data-testid=chat-scroll] [data-chat-entry-id]").length > 20, null, { timeout: 30_000 });
    result.switchBackMs = Date.now() - t0;
  }
  const slow = await page.evaluate(() => window.__events.map((e) => e.duration));
...
async function liveWindow(seconds) {
  const driver = spawn(process.execPath, [path.join(here, "drive.mjs"), "live", "--api", args.api, "--root", ROOT, "--finding", setup.findings[0], "--seconds", String(seconds)], { stdio: "ignore" });
  ...
  for (let t = step; t <= seconds; t += step) {
    ...
    out({
      ...
      entryKBPerSec: Math.round((sse.entryBytes - prevSse.entryBytes) / step / 1024),
